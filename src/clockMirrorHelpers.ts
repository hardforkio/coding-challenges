...
export const parseTimestamp = (stringTime: string): number => {
  ...
  /*FIXME: When input is invalid, should we throw an exception or return some kind of special value, and which value (undefined?)?
    Also, if using special value, how to fit the function into compositions chains?*/

  const h = parseInt(stringTime.substring(0, 3));
  const m = parseInt(stringTime.substring(3, 5));
